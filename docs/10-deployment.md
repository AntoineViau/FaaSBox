...
to any S3-compatible storage, and to put it back before anything reads it on the
next boot.

**It is entirely optional.** Leave the variables unset and FaaSBox starts
normally, from the local file alone, which is what the persistent volume above
is for.

It has a page of its own, because turning it on is five variables and knowing
what it does for you is rather more: see
[14 - Litestream Replication](14-litestream-replication.md) for the variables,
the prefix your data lives under, what happens at every boot, and the one file
that is deliberately not mirrored.
...
3.  **Litestream**: a continuous mirror to S3-compatible storage, always
    current, which is a different guarantee from a periodic dump — there is no
    window during which the last hour is missing. See
    [14 - Litestream Replication](14-litestream-replication.md).
