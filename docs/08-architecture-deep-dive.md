...
Encrypted with `FAASBOX_ENCRYPTION_KEY`, per column: the name of each function, its code, its `package.json` and lockfile, the output of its dependency install, the name, schedule and payload of each trigger, the three output fields of every execution, the label and visible prefix of each API key, and the name, identifier and redirect URLs of each authorized agent. The key never encrypts anything directly — two subkeys are derived from it by HKDF-SHA256, each under a label of its own: one encrypts, the other computes the fingerprints below.

**Hashes** are deliberately not encrypted, and stay hashes — the SHA-256 of an API key, the token fingerprints of an agent: encrypting one would turn the database plus the key back into a usable credential.

Two columns are **queried in SQL**, which no ciphertext survives on its own: the name of a function, resolved by every `/invoke/{name}` call, and the identifier of an OAuth client. They are encrypted all the same, and a **blind index** beside each — a keyed fingerprint of the value, in the clear — is what the server searches and what carries the uniqueness of a function name. It is keyed rather than a plain hash for a reason worth knowing: function names are short and guessable, and a plain hash of one would fall to a dictionary in seconds from a stolen copy. The key is not in the database, which is exactly why that copy is not enough — and why the key must not travel with your backups. A fingerprint still reveals that two identical values are identical; that is what being able to find a row costs.
