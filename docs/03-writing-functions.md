...
**The body and the headers are saved with the function.** They are fields of it, like your script: switch to another function and you see that function's sample, come back and yours is where you left it. A rename carries it along, deleting the function takes it with it. The two rows and the body described above are written when the Editor creates the function, exactly as the example script is — so they are yours from the start, and clearing one clears it for good rather than bringing the example back.

The Editor is not the only author. A function written through the [management API](09-api-reference.md#3-manage-functions) or by an [agent](13-ai-agents.md) carries the sample its author sent, and none when they sent none — nothing is substituted for what nobody wrote. That is worth knowing when you open a function you did not write: the call in the panel is the one whoever wrote it says the function expects.

The body and each header list are capped at **16,384 characters**, which is generous for an example of a call and deliberately too small to be an archive of one — the sample travels with the function every time the Editor lists them. Over the cap, the save is refused and the message names the field.
