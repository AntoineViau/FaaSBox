...
**The body and the headers are saved with the function.** They are fields of it, like your script: switch to another function and you see that function's sample, come back and yours is where you left it. A rename carries it along, deleting the function takes it with it. The two rows and the body described above are written when the Editor creates the function, exactly as the example script is — so they are yours from the start, and clearing one clears it for good rather than bringing the example back. A function created through the management API has no sample at all, for the same reason it has no example script: whoever wrote it supplied their own.
